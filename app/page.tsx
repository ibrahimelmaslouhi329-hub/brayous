...
export default function Page() {
  ...
  return (
    <div style={{ backgroundColor: bg, color: text, minHeight: '100vh', direction: 'rtl', transition: '0.3s' }}>
      
      {/* Header مع زر الوضع المظلم */}
      ...
      {selectedItem && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.9)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '15px' }}>
          <div style={{ backgroundColor: cardBg, padding: '20px', borderRadius: '25px', width: '100%', maxWidth: '400px', position: 'relative', color: text }}>
            <button onClick={() => setSelectedItem(null)} style={{ position: 'absolute', top: '15px', left: '15px', backgroundColor: '#ff0000', color: '#fff', border: 'none', borderRadius: '50%', width: '35px', height: '35px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <X size={20} />
            </button>
            ...
            <p style={{ color: isDarkMode ? '#ccc' : '#666', fontSize: '0.9rem' }}>{selectedItem.description}</p>
            <a href={`https://wa.me/${whatsappNumber}?text=طلب: ${selectedItem.name}`} style={{ display: 'block', background: '#25D366', color: '#fff', textAlign: 'center', padding: '14px', borderRadius: '15px', textDecoration: 'none', fontWeight: 'bold', marginTop: '15px' }}>
              طلب عبر واتساب
            </a>
          </div>
        </div>
