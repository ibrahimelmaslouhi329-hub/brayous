    ...
    <div style={{ backgroundColor: theme.bg, color: theme.text, minHeight: '100vh', direction: 'rtl', transition: '0.3s', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: theme.bg, zIndex: 1000, overflowY: 'auto' }}>
          ...
          <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '15px' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px' }}>
              ...
              {/* صورة المنتج (الجنب الأيمن) */}
              <div style={{ flex: '1 1 400px', maxWidth: '100%' }}>
                <img src={mainImage} alt="" style={{ width: '100%', borderRadius: '15px', backgroundColor: '#fff', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }} />
              ...
              {/* المعلومات والوصف والطلب (الجنب الأيسر) */}
              <div style={{ flex: '1 1 350px' }}>
                <h2 style={{ fontSize: '1.6rem', fontWeight: '800', margin: 0 }}>{selectedItem.name}</h2>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '15px', margin: '15px 0' }}>
                  <span style={{ color: theme.redBrand, fontSize: '2.2rem', fontWeight: '900' }}>{selectedItem.price} DH</span>
                ...
                <div style={{ marginBottom: '25px' }}>
                  <h4 style={{ marginBottom: '10px', color: theme.secondaryText, fontSize: '0.9rem' }}>وصف المنتج:</h4>
                  <p style={{ fontSize: '1rem', lineHeight: '1.8', whiteSpace: 'pre-line', margin: 0 }}>
                    {selectedItem.description}
                  </p>
                </div>

                <button onClick={() => addToCart(selectedItem)} style={{ width: '100%', backgroundColor: theme.redBrand, color: '#fff', padding: '20px', borderRadius: '15px', border: 'none', fontWeight: 'bold', fontSize: '1.2rem', cursor: 'pointer', transition: '0.2s' }}>
                  إضافة إلى السلة 🛒
                </button>

                {/* التقييمات بسيطة فجنب الوصف */}
                <div style={{ marginTop: '30px' }}>
                  <h4 style={{ marginBottom: '15px', borderTop: '1px solid #eee', paddingTop: '15px' }}>آراء الزبناء ⭐</h4>
                  ...
                  <div style={{ marginTop: '15px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <textarea placeholder="تعليقك" value={newReview.comment} onChange={(e)=>setNewReview({...newReview, comment:e.target.value})} style={{padding:'10px', borderRadius:'8px', border:'1px solid #eee', fontSize:'0.8rem', minHeight:'50px', backgroundColor: theme.card, color: theme.text}} />
                    <button onClick={() => handleAddReview(selectedItem._id)} style={{ backgroundColor: '#000', color: '#fff', border: 'none', padding: '8px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 'bold' }}>إرسال التقييم</button>
                  </div>
                </div>
              </div>
          ...
          <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
            {cart.map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px 0', borderBottom: '1px solid #eee' }}>
                <div><h4>{item.name}</h4><p style={{color: theme.redBrand, fontWeight: 'bold'}}>{item.price} DH</p></div>
                <button onClick={() => {const n=[...cart]; n.splice(i,1); setCart(n);}} style={{color:'red', border:'none', background:'none'}}><Trash2 size={22}/></button>
              </div>
            ))}
            {cart.length > 0 && <button onClick={() => {setIsCartOpen(false); setShowOrderForm(true);}} style={{ width: '100%', backgroundColor: theme.redBrand, color: '#fff', padding: '18px', borderRadius: '15px', marginTop: '20px', fontWeight: 'bold' }}>إتمام الطلب 🚀</button>}
          </div>
        </div>
      )}
      ...
      {showOrderForm && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '15px' }}>
          <div style={{ backgroundColor: theme.card, padding: '25px', borderRadius: '25px', width: '100%', maxWidth: '400px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                ...
                <input type="text" placeholder="العنوان" onChange={(e)=>setOrderInfo({...orderInfo, address:e.target.value})} style={{width:'100%', padding:'15px', borderRadius:'12px', border:'1px solid #ddd', color:'#000'}} />
                <button onClick={() => window.open(`https://wa.me/212601042910?text=طلب جديد من الموقع: ${cart.map(i=>i.name).join(', ')}`, '_blank')} style={{ width: '100%', backgroundColor: '#25D366', color: '#fff', padding: '18px', borderRadius: '12px', border: 'none', fontWeight: 'bold', marginTop: '10px' }}>تأكيد عبر واتساب</button>
                <button onClick={()=>setShowOrderForm(false)} style={{background:'none', border:'none', color: '#999', marginTop: '10px'}}>إغلاء</button>
            </div>
          </div>
        </div>
      )}